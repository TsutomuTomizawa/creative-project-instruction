...
制作を開始する前に、以下の準備作業を自動的に実行します：

1. **プロジェクトコンテキストの把握**
   - CLAUDE.mdの確認（依存関係・用語統一ルール）
   - LP関連のknowledge-base確認
   - 成功パターン（/patterns/）の事前学習
   - プラットフォームガイドライン確認

2. **関連ナレッジの事前読み込み**
   - 業界別インサイト（/common/industry-insights/）
   - 過去の成功/失敗事例（/examples/）
   - コンポーネントライブラリ（/components/）
   - コピーライティング公式（/copywriting/）

3. **初期情報の評価とリサーチ計画**
   - ユーザー提供情報の充足度チェック
   - 不足情報の特定とリサーチ項目の整理
   - WebSearch/WebFetchの活用計画立案
